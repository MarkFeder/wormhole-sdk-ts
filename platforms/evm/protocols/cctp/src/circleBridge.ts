  ...
  implements CircleBridge<N, C>
{
  ...
  async getBurnLimit(token: ChainAddress): Promise<bigint> {
    // ponytail: minimal ABI for the one view method; the generated TokenMessenger
    // bindings don't include it (autogenerated from the deployment artifacts)
    const tokenMessenger = new ethers.Contract(
      this.tokenMessenger.target,
      ['function burnLimitsPerMessage(address token) view returns (uint256)'],
      this.provider,
    ) as unknown as { burnLimitsPerMessage(token: string): Promise<bigint> };
    return await tokenMessenger.burnLimitsPerMessage(token.address.toString());
  }
